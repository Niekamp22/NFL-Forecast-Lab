...
from ..player_views import POSITION_STATS
...
def grade_player_rows(predictions,schedule,pbp,stats):
    ...
    result_columns=finals[['game_id','status','home_score','away_score']].copy()
    result_columns['game_result_status']=result_columns.status
    for column in ['home_score','away_score']:
        result_columns[column]=result_columns[column].where(result_columns.status.eq('graded'))
    out=out.merge(result_columns,on='game_id',how='left',validate='many_to_one')
...
def grade_role_week(client,season,week,refresh=False,defense=False,current=False):
    root=client.root
    folder,meta,predictions=checked_player_snapshot(root/('player_role_defense_forecasts' if defense else 'player_role_forecasts')/str(season)/f'week_{week:02d}',current=current)
    refs=[];frames={}
    ...
    relevant=grades.loc[grades.apply(lambda r:r.stat in POSITION_STATS.get(r.position,[]),axis=1)]
    metrics=relevant[relevant.status.eq('graded')].groupby(['position','stat']).error.agg(observations='size',mae=lambda x:x.abs().mean(),bias='mean').reset_index()
    ...
    run=dict(created_at=str(now),snapshot=str(folder.resolve()),snapshot_hash=meta['predictions_sha256'],sources=refs,
             selection='displayed current-policy snapshot' if current else 'earliest pre-kickoff snapshot',
             forecast_created_at=meta['created_at'],role_policy=meta.get('role_policy'),
             statuses=grades.status.value_counts().to_dict(),graded=int(grades.status.eq('graded').sum()),
             relevant_statuses=relevant.status.value_counts().to_dict(),
